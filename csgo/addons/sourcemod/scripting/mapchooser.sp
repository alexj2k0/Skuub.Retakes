...
public void OnPluginStart()
{
	...
	g_Cvar_IncludeMaps = CreateConVar("sm_mapvote_include", "5", "Specifies how many maps to include in the vote. 0 includes every available map.", _, true, 0.0);
	g_Cvar_IncludeMaps.SetBounds(ConVarBound_Lower, true, 0.0);
	g_Cvar_IncludeMaps.SetBounds(ConVarBound_Upper, false);
	g_Cvar_ExcludeMaps.AddChangeHook(ConVarChange_RebuildNextVote);
	g_Cvar_IncludeMaps.AddChangeHook(ConVarChange_RebuildNextVote);
...
public void ConVarChange_RebuildNextVote(ConVar convar, const char[] oldValue, const char[] newValue)
{
	if (g_MapList != null && !g_HasVoteStarted)
	{
		CreateNextVote();
	}
}
...
void InitiateVote(MapChange when, ArrayList inputlist=null)
{
	...
	if (inputlist == null)
	{
		...
		if (voteSize <= 0)
		{
			voteSize = nominateCount + g_NextMapList.Length;
		}
...
void CreateNextVote()
{
	...
	int includeMaps = g_Cvar_IncludeMaps.IntValue;
	int limit = (includeMaps <= 0 || includeMaps > tempMaps.Length) ? tempMaps.Length : includeMaps;
