...
int g_WinStreak = 0;
int g_WinStreakTeam = CS_TEAM_NONE;
int g_RoundCount = 0;
bool g_HalfTime;
...
public void OnPluginStart() {
    ...
    g_hRoundsToScramble = CreateConVar("sm_retakes_scramble_rounds", "5", "Consecutive wins by one team to cause a team scramble.");
...
static void TeamWon(int winner) {
    if (g_WinStreakTeam == winner) {
        g_WinStreak++;
    } else {
        if (g_WinStreak >= 3) {
            Retakes_MessageToAll("{GREEN}%s{NORMAL} ended a {GREEN}%d{NORMAL}-round %s win streak.",
                TEAMSTRING(winner), g_WinStreak, TEAMSTRING(g_WinStreakTeam));
        }

        g_WinStreakTeam = winner;
        g_WinStreak = 1;
    }
    ...
    if (ScramblesEnabled()) {
        ...
        if (g_WinStreak >= toScramble) {
            g_ScrambleSignal = true;
            Retakes_MessageToAll("{GREEN}%s{NORMAL} won {GREEN}%d{NORMAL} rounds in a row. {PURPLE}Teams are being scrambled!",
                TEAMSTRING(winner), g_WinStreak);
            g_WinStreak = 0;
            g_WinStreakTeam = CS_TEAM_NONE;
        } else if (g_WinStreak >= 3 && g_WinStreak >= toScramble - 3) {
            Retakes_MessageToAll("{GREEN}%s{NORMAL} has won {GREEN}%d{NORMAL} rounds in a row - if they win {GREEN}%d more, {PURPLE}teams will be scrambled.",
                TEAMSTRING(winner), g_WinStreak, toScramble - g_WinStreak);
        } else if (g_WinStreak >= 3) {
            Retakes_MessageToAll("{GREEN}%s{NORMAL} has won {GREEN}%d{NORMAL} rounds in a row!",
                TEAMSTRING(winner), g_WinStreak);
        }
    } else if (g_WinStreak >= 3) {
        Retakes_MessageToAll("{GREEN}%s{NORMAL} has won {GREEN}%d{NORMAL} rounds in a row!",
